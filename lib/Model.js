...
De.prototype = {
    ...
    pull: function (provs, fn, ctx) {
        ...
        //  Как обрабатывать ошибки которые произойдут в колбэке???
    ...
    __pull__: function (provs, ctx, cache) {
        ...
        for ( i = 0, l = provs.length; i < l; i += 1) {
            ...
            if ( !hasProperty.call(this.__provs__, id) ) {
    ...
    __invoke__: function (id, ctx, cache) {
        ...
        promise = JSPromise.resolve(this.__provs__[id]).then(function (decl) {
        ...
        promise.then(function (value) {
            de.__emitter__.emit(De.events.DATA_ACCEPTED, id, value);
        }, function (reason) {
            de.__emitter__.emit(De.events.DATA_REJECTED, id, reason);
        });
